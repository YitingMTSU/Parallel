...
int main(int argc, char *argv[])
{
  ...
  //while(max_err > eps)
       ...
       if(myrank == 0)
	{
	   ...
	   for(i=1;i<numranks;i++)
	    {
	      ...
	      MPI_Send(&newM[start_row][0],(end_row-start_row)*num_cols,MPI_DOUBLE,id+1,tag,MPI_COMM_WORLD);
	  ...
	  MPI_Recv(&newM[start_row][0],(end_row-start_row)*num_cols,MPI_DOUBLE,0,tag,MPI_COMM_WORLD,&status);
	  for (i=0; i<num_rows;i++)
	    {
	      printf("(%d): ",i);
	      for (j=0; j<num_cols; j++)
	  ...
	  for (i=start_row+1;i<end_row;i++)
		...
		{
		  average = (new[i+1][j] + new[i-1][j] + new[i][j+1] +\
			     + new[i][j-1])/4.0;
		  new[i][j] = average;
		}
	    }
